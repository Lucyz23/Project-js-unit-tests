...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(Object.keys(createMenu({food: {}, drink: {}}).fetchMenu())).toEqual(expect.arrayContaining(['food', 'drink']));
    ...
    const objeto3 = {alguma: {}, coisa: {} };
    const objetoRetornado3 = createMenu(objeto3);
    expect(objetoRetornado3.fetchMenu()).toEqual(objeto3);
    ...
    objetoRetornado.order('outra coisa');
    objetoRetornado.order('outra coisa');
    expect(objetoRetornado.consumption).toStrictEqual(['coxinha', 'outra coisa', 'outra coisa']);

    expect(objetoRetornado.consumption.length).toBe(3);

    const objeto11 = {coxinha: {price: 3}, refri: {price: 5}};
    const teste11 = createMenu(objeto11);
    teste11.order ('coxinha');
    teste11.order ('refri');
    teste11.order ('refri');
    expect(teste11.pay()).toBeCloseTo(14.3);
